    ...
    <div>
        <v-card>
            <v-card-title class="d-flex flex-column align-start">
                <p>Question {{allQuizzes[currentQuestion].quiz.length}}</p>
                <!-- <p>{{questions[currentQuestion]}}</p> -->
            </v-card-title>
            <v-divider/>
            <v-card-text>
            
                <h3 class="my-5" v-for="(answer, ind) in allQuizzes[0].quiz[currentQuestion].answers" :key="ind" :style="{color:answer.answered?'green':'red'}">
                    {{answer.answer}}
                </h3>
                <p>
